...
// ─── FAKE BRANDS ─────────────────────────────────────────────────────────────
const BRAND_MAP = {
  photo:     ["Memova", "Framely", "Veloria", "Picsora", "Lumencraft"],
  game:      ["Hexplay", "Boardica", "Puzzloft", "Diceborn", "Cardwell"],
  essential: ["Havenist", "Daylume", "Nordia", "Softerra", "Velvora"],
  gift:      ["Giftora", "Wraply", "Ritualco", "Bloomset", "Knackery"],
  general:   ["Printworks", "Memova", "Framely", "Hexplay", "Havenist",
               "Giftora", "Veloria", "Boardica", "Daylume", "Nordia"],
};
...
function pickVendor(handle, type) {
  const t = (type || "").toLowerCase();
  let pool;
  if (/photo|album|book|guest|wedding|baby|dog/.test(t))   pool = BRAND_MAP.photo;
  else if (/game|puzzle|chess|card|dice|tennis/.test(t))   pool = BRAND_MAP.game;
  else if (/essential|candle|fan|tool|pencil/.test(t))     pool = BRAND_MAP.essential;
  else if (/gift|jewelry|wrap|curated|set/.test(t))        pool = BRAND_MAP.gift;
  else pool = BRAND_MAP.general;
  return pool[Math.floor(seededRand(handle) * pool.length)];
}

// ~20% New, ~15% Best Seller, random per product (deterministic by handle)
function addPromoTags(tags, handle) {
  const result = [...tags];
  if (seededRand(handle + "_new") < 0.20) result.push("New");
  if (seededRand(handle + "_bs")  < 0.15) result.push("Best Seller");
  return result;
}
...
function productToRows(p) {
  ...
  const firstRow = {
    ...
    "Vendor":                    pickVendor(p.handle ?? "", p.type ?? ""),
    "Product Category":          p.category ?? "",
    "Type":                      p.type ?? "",
    "Tags":                      addPromoTags(p.tags ?? [], p.handle ?? "").join(", "),
    ...
    "Color Variants (product.metafields.custom.color_variants)":
      mf.color_variants ?? "",
    "Product Information (product.metafields.custom.product_information)":
      (mf.product_information ?? "").replace(/^Vendor: .+$/m, `Vendor: ${pickVendor(p.handle ?? "", p.type ?? "")}`),
    "Board game mechanics (product.metafields.shopify.board-game-mechanics)":
